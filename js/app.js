...
function toggleMoroshka(){
  ...
  document.querySelectorAll(".pv-item").forEach(function(row){
    var nameEl=row.querySelector(".pv-name");if(!nameEl)return;
    var name=nameEl.textContent;
    servicesData.forEach(function(cat){
      cat.items.forEach(function(it){
        if(it.n===name){
          var p=hasMoroshka&&it.m!=null?it.m:it.p;
          var priceEl=row.querySelector(".pv-price");
          if(priceEl){
            var save=hasMoroshka&&it.m!=null?'<span class="pv-save">−'+(it.p-it.m)+'₽</span>':"";
            priceEl.innerHTML=p+' ₽ '+save;
          }
          var btn=row.querySelector(".pv-add");
          if(btn){
            var oc=btn.getAttribute("onclick");
            if(oc)btn.setAttribute("onclick",oc.replace(/,\d+,this/,","+p+",this"));
          }
        }
      });
    });
...
function addToCart(id,name,price,btn,base,mor){
  ...
  saveCart();updateBadge();updBtBadge();renderCart();
...
function clearCart(){cart=[];saveCart();updateBadge();updBtBadge();renderCart();closeCart();}
...
function doSendOrder(){
  ...
  cart=[];saveCart();updateBadge();updBtBadge();renderCart();closeCart();
...
function showMainMenu(){
  ...
  w.innerHTML=
    '<div class="greet-anim"><div class="ga-orb ga-1"></div><div class="ga-orb ga-2"></div><div class="ga-orb ga-3"></div>'+
    '<div class="ga-body"><span class="ga-hi">'+gr+',</span><div class="ga-name">'+clientName+'</div><span class="ga-sub">Чем могу помочь?</span></div></div>'+

    '<div class="hm-list">'+
    '<button class="hm" data-a="assistant"><span class="hm-i" style="background:linear-gradient(135deg,#6366f1,#4f46e5)"><img src="img/bot-avatar.jpg" style="width:26px;height:26px;border-radius:50%;object-fit:cover"></span><div class="hm-t"><b>Помощник «Гармония»</b><span>Задать вопрос</span></div><span class="hm-a">›</span></button>'+
    '<button class="hm" data-a="services"><span class="hm-i" style="background:linear-gradient(135deg,#1B8585,#0d6b6b)">📋</span><div class="hm-t"><b>Прейскурант</b><span>Услуги и цены</span></div><span class="hm-a">›</span></button>'+
    '<button class="hm" data-a="booking"><span class="hm-i" style="background:linear-gradient(135deg,#f59e0b,#d97706)">📅</span><div class="hm-t"><b>Записаться</b><span>К специалисту</span></div><span class="hm-a">›</span></button>'+
    '<button class="hm" data-a="staff"><span class="hm-i" style="background:linear-gradient(135deg,#10b981,#059669)">👥</span><div class="hm-t"><b>Сотрудники</b><span>Справочник</span></div><span class="hm-a">›</span></button>'+
    '</div>'+

    '<div class="hm-sec">Услуги и информация</div>'+
    '<div class="hm-grid">'+
    '<button class="hg" data-a="homeWorker"><span class="hg-i" style="background:linear-gradient(135deg,#ef4444,#dc2626)">🏠</span><b>На дом</b></button>'+
    '<button class="hg" data-a="callback"><span class="hg-i" style="background:linear-gradient(135deg,#1B8585,#14b8a6)">📞</span><b>Звонок</b></button>'+
    '<button class="hg" data-a="events"><span class="hg-i" style="background:linear-gradient(135deg,#f59e0b,#d97706)">🎟️</span><b>События</b></button>'+
    '<button class="hg" data-a="news"><span class="hg-i" style="background:linear-gradient(135deg,#3b82f6,#2563eb)">📰</span><b>Новости</b></button>'+
    '<button class="hg" data-a="contacts"><span class="hg-i" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed)">📍</span><b>Контакты</b></button>'+
    '<button class="hg" data-a="faq"><span class="hg-i" style="background:linear-gradient(135deg,#ec4899,#db2777)">❓</span><b>FAQ</b></button>'+
    '<button class="hg" data-a="emergency"><span class="hg-i" style="background:linear-gradient(135deg,#ef4444,#b91c1c)">🆘</span><b>SOS</b></button>'+
    '<button class="hg" data-a="liveChat"><span class="hg-i" style="background:linear-gradient(135deg,#6366f1,#4f46e5)">💬</span><b>Оператор</b></button>'+
    '</div>';
  ...
  w.querySelectorAll("[data-a]").forEach(function(btn){
    var a=btn.dataset.a;if(acts[a])btn.onclick=function(){acts[a]();};
  });
...
function showServices(){
  ...
  chatEl.innerHTML="";
  var pg=document.createElement("div");pg.className="page-view";
  var html='<div class="pv-header"><h2>Прейскурант услуг</h2><div class="pv-search"><input type="text" placeholder="Поиск услуг..." oninput="filterPV(this.value)"></div></div>';
  html+='<div class="pv-tabs">';
  servicesData.forEach(function(cat,i){
    html+='<button class="pv-tab'+(i===0?" active":"")+'" onclick="showPVCat('+cat.id+',this)">'+cat.icon+'</button>';
  });
  html+='</div><div class="pv-list" id="pvList">';
  servicesData.forEach(function(cat,ci){
    html+='<div class="pv-section'+(ci>0?" gone":"")+'" data-cid="'+cat.id+'"><div class="pv-cat-name">'+cat.icon+" "+cat.name+'</div>';
    cat.items.forEach(function(it){
      var p=hasMoroshka&&it.m!=null?it.m:it.p;
      var save=hasMoroshka&&it.m!=null?'<span class="pv-save">−'+(it.p-it.m)+'₽</span>':"";
      html+='<div class="pv-item"><div class="pv-info"><div class="pv-name">'+it.n+'</div><div class="pv-price">'+p+' ₽ '+save+'</div></div><button class="pv-add" onclick="addToCart(0,\''+it.n.replace(/'/g,"\\'")+'\',' +p+',this,'+it.p+','+(it.m!=null?it.m:"null")+')">+</button></div>';
    });
    html+='</div>';
  });
  html+='</div>';
...
function showPVCat(id,btn){
  document.querySelectorAll(".pv-tab").forEach(function(t){t.classList.remove("active");});
  btn.classList.add("active");
  document.querySelectorAll(".pv-section").forEach(function(s){s.classList.toggle("gone",+s.dataset.cid!==id);});
}
function filterPV(q){
  q=q.toLowerCase();
  document.querySelectorAll(".pv-section").forEach(function(s){s.classList.remove("gone");});
  document.querySelectorAll(".pv-tab").forEach(function(t){t.classList.remove("active");});
  document.querySelectorAll(".pv-item").forEach(function(it){
    it.style.display=(!q||it.querySelector(".pv-name").textContent.toLowerCase().includes(q))?"":"none";
  });
}
...
function showStaff(){
  clearActions();setNav(true);
  if(!staffData.length){addMsg("Нет данных для этого филиала.",true);return;}
  chatEl.innerHTML="";
  var pg=document.createElement("div");pg.className="page-view";
  var depts={};staffData.forEach(function(s){if(!depts[s.dept])depts[s.dept]=[];depts[s.dept].push(s);});
  var html='<div class="pv-header"><h2>Сотрудники</h2></div>';
  Object.keys(depts).forEach(function(d){
    html+='<div class="pv-cat-name">'+d+' ('+depts[d].length+')</div>';
    depts[d].forEach(function(p){
      var ini=p.name.split(" ").slice(0,2).map(function(w){return w[0];}).join("");
      html+='<div class="staff-row"><div class="sr-ava">'+ini+'</div><div class="sr-info"><b>'+p.name+'</b><div>'+p.pos+'</div>'+(p.ext?'<div>📞 '+MAIN_PHONE+' доб. '+p.ext+'</div>':'')+(p.email?'<div>✉️ '+p.email+'</div>':'')+'</div></div>';
    });
  });
  pg.innerHTML=html;
  chatEl.appendChild(pg);
