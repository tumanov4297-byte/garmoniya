...
function showMainMenu(){
  ...
  w.innerHTML=
    '<div class="h-hero"><div class="h-hero-deco"></div><div class="h-hero-body">'+
    '<span class="h-hero-hi">'+gr.replace(/[^\s]+/,"").trim()+',</span>'+
    '<div class="h-hero-name">'+clientName+'</div>'+
    '<span class="h-hero-sub">Чем могу помочь?</span>'+
    '</div><div class="h-hero-ava"><img src="img/bot-avatar.jpg" alt=""></div></div>'+

    '<div class="h-cards">'+
    '<button class="hc" data-act="assistant"><div class="hc-i hc-purple"><img src="img/bot-avatar.jpg" class="hc-av"></div><div class="hc-t"><b>Помощник «Гармония»</b><span>Задать вопрос ассистенту</span></div><span class="hc-a">&rsaquo;</span></button>'+
    '<button class="hc" data-act="services"><div class="hc-i hc-green-v"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg></div><div class="hc-t"><b>Прейскурант</b><span>Услуги и цены</span></div><span class="hc-a">&rsaquo;</span></button>'+
    '<button class="hc" data-act="booking"><div class="hc-i hc-orange-v"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><path d="M8 14h.01M12 14h.01M16 14h.01"/></svg></div><div class="hc-t"><b>Записаться</b><span>Запись к специалисту</span></div><span class="hc-a">&rsaquo;</span></button>'+
    '</div>'+
    ...
    '<div class="h-sec"><span>Кабинет</span></div>'+
    '<div class="h-grid">'+
    '<button class="hg" data-act="cabinet"><div class="hg-i" style="background:linear-gradient(135deg,#0ea5e9,#0284c7)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><b>Мой кабинет</b><span>Заявки и записи</span></button>'+
    '<button class="hg" data-act="cart"><div class="hg-i" style="background:linear-gradient(135deg,#f59e0b,#d97706)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6"/></svg></div><b>Корзина</b><span>'+cart.reduce(function(s,i){return s+i.qty;},0)+' услуг</span></button>'+
    '<button class="hg" data-act="contacts"><div class="hg-i" style="background:linear-gradient(135deg,#10b981,#059669)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></div><b>Контакты</b><span>Телефон, адрес</span></button>'+
    '<button class="hg" data-act="liveChat"><div class="hg-i" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></div><b>Оператор</b><span>Макс, телефон</span></button>'+
    '</div>';
  ...
  w.querySelectorAll("[data-act]").forEach(function(btn){
    var a=btn.dataset.act;if(actions[a])btn.onclick=function(){actions[a]();};
  });
  if(typeof showQuestionnaire==="function")showQuestionnaire(function(){if(typeof showOnboarding==="function")showOnboarding();});
}
...
function showServices(){
  ...
  var html='Выберите категорию:<div class="chat-menu">';
...
function openProfile(){
  ...
  modal.innerHTML=`<div class="mc">
    <div class="cab-header">
      <div class="cab-ava"><img src="img/bot-avatar.jpg" alt=""></div>
      <div class="cab-info">
        <div class="cab-name">${clientName}</div>
        <div class="cab-phone">${clientPhone}</div>
      </div>
    </div>
    <div class="mo-info">
      ...
      ${(function(){try{var p=JSON.parse(localStorage.getItem("userProfile")||"{}");var r="";var cats={pensioner:"Пенсионер",disabled:"Инвалид",family:"Семья с детьми",large_family:"Многодетная семья",veteran:"Ветеран",other:"Другое"};if(p.category)r+='<p><span class="lbl">Категория:</span><span class="val">'+(cats[p.category]||p.category)+'</span></p>';if(p.address)r+='<p><span class="lbl">Адрес:</span><span class="val">'+p.address+'</span></p>';return r;}catch(e){return "";}})()}
    ...
    <div class="cab-actions">
      <button class="cab-btn" onclick="editMyData()">✏️ Данные</button>
      <button class="cab-btn" onclick="editQuestionnaire()">📋 Анкета</button>
...
function showAuth(){
  ...
  const modal=document.createElement("div");modal.className="auth-ovl";
  setTimeout(function(){modal.insertAdjacentHTML("afterbegin",'<div class="auth-bg"><div class="orb o1"></div><div class="orb o2"></div><div class="orb o3"></div><div class="orb o4"></div><div class="orb o5"></div></div>');},0);modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.setAttribute("aria-label","Вход в систему");
